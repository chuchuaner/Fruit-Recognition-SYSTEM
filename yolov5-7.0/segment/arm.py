...
class armControl():
    def __init__(self):
        ...
        # 记录三种水果被抓取的次数，并根据测试不同设置不同的命令
        self.countFruitNumber = np.zeros_like(np.arange(0,3))
        # 记录不同的抓取次数对应的指令          最终的指令由水果种类和对应的抓取次数共同决定
        # self.eachCountCmd = [[
        #     bytes.fromhex("ff 02 08 {}".format(self.angleToHex(70))),
        #         bytes.fromhex("ff 02 05 {}".format(self.angleToHex(70))),
        #         bytes.fromhex("ff 02 04 {}".format(self.angleToHex(0))),
        #         bytes.fromhex("ff 02 05 {}".format(self.angleToHex(122))),
        #         bytes.fromhex("ff 02 08 {}".format(self.angleToHex(0))),
        # ],[],[]]
        self.eachCountCmd = [[
            bytes.fromhex("ff 02 08 {}".format(self.angleToHex(180))),
            bytes.fromhex("ff 02 05 {}".format(self.angleToHex(70))),
            bytes.fromhex("ff 02 04 {}".format(self.angleToHex(0))),
            bytes.fromhex("ff 02 06 {}".format(self.angleToHex(6))),
            bytes.fromhex("ff 02 07 {}".format(self.angleToHex(174))),
            bytes.fromhex("ff 02 05 {}".format(self.angleToHex(112))),
            bytes.fromhex("ff 02 08 {}".format(self.angleToHex(120))),
        ],
            [bytes.fromhex("ff 02 08 {}".format(self.angleToHex(180))),
             bytes.fromhex("ff 02 05 {}".format(self.angleToHex(70))),
            bytes.fromhex("ff 02 04 {}".format(self.angleToHex(0))),
            bytes.fromhex("ff 02 06 {}".format(self.angleToHex(42))),
            bytes.fromhex("ff 02 07 {}".format(self.angleToHex(174))),
             bytes.fromhex("ff 02 05 {}".format(self.angleToHex(130))),
            bytes.fromhex("ff 02 08 {}".format(self.angleToHex(120))),],
            [
                bytes.fromhex("ff 02 08 {}".format(self.angleToHex(180))),
                bytes.fromhex("ff 02 05 {}".format(self.angleToHex(70))),
                bytes.fromhex("ff 02 04 {}".format(self.angleToHex(0))),
                bytes.fromhex("ff 02 06 {}".format(self.angleToHex(0))),
                bytes.fromhex("ff 02 07 {}".format(self.angleToHex(111))),
                bytes.fromhex("ff 02 05 {}".format(self.angleToHex(146))),
                bytes.fromhex("ff 02 08 {}".format(self.angleToHex(120))),
            ]]
    ...
    # 输入水果的种类，放入不同的盘子   水果种类为：0~2
    def putFruit(self,fruitClass):
        if fruitClass>=3:
            print("fruit type error")
            return
        cmd = copy(self.eachCountCmd[self.countFruitNumber[fruitClass]])
        cmd[2] = bytes.fromhex("ff 02 04 {}".format(self.angleToHex(50*fruitClass)))
        # cmd = [
        #     bytes.fromhex("ff 02 08 {}".format(self.angleToHex(70))),
        #     bytes.fromhex("ff 02 05 {}".format(self.angleToHex(70))),
        #     bytes.fromhex("ff 02 04 {}".format(self.angleToHex(70*fruitClass))),
        #     bytes.fromhex("ff 02 05 {}".format(self.angleToHex(122))),
        #     bytes.fromhex("ff 02 08 {}".format(self.angleToHex(0))),
        # ]
        print(cmd)
        for i in range(len(cmd)):
            self.ser.write(cmd[i])
            time.sleep(3.5)
        self.countFruitNumber[fruitClass]+=1
        self.countFruitNumber[fruitClass] %= 3
    ...
    def resetArmsAngle(self):
        ...
        self.controlCount[8] = 100 // self.perAngle
        cmd = [
